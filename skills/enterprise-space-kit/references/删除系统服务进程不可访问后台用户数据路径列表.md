...
```
2.
调用接口 [deleteRestrictedAccessBackgroundUserdataProcessList](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/enterprisespace-spacemanager#deleterestrictedaccessbackgrounduserdataprocesslist) ，删除系统服务进程不可访问后台用户数据路径列表，并且查看打印信息。
```typescript
